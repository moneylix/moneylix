...
export default function CategoriesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) { showT(editingCategory ? t('categories.updated') : t('categories.created')); setShowModal(false); setEditingCategory(null); resetForm(); fetchCategories() }
  }
  ...
  const handleDelete = async (cat: Category) => {
    if (!confirm(`${t('categories.deleteConfirmPrefix')} "${cat.name}"?`)) return
    const token = localStorage.getItem('moneylix_session_token') ?? ''
    const res = await fetch(`/api/categories/${cat.id}`, { method: 'DELETE', headers: token ? { Authorization: `Bearer ${token}` } : {} })
    if (res.ok) { showT(t('categories.deleted')); fetchCategories() } else { const d = await res.json(); showT(d.error || t('categories.cannotDelete')) }
  }
  ...
  const CatRow = ({ cat }: { cat: Category }) => {
    ...
    return (
      <tr className="border-b border-black/5 hover:bg-black/5 transition">
        ...
        <td className="px-3 py-1.5"><span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${cat.type === 'credit' ? 'bg-lime-100 text-lime-700' : cat.type === 'both' ? 'bg-cyan-100 text-cyan-700' : 'bg-rose-100 text-rose-600'}`}>{cat.type === 'credit' ? t('transactions.credit').toLowerCase() : cat.type === 'both' ? t('categories.both').toLowerCase() : t('transactions.debit').toLowerCase()}</span></td>
        <td className="px-3 py-1.5">
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-base font-bold text-neutral-900">{t('nav.categories')}</h1>
          <p className="text-[10px] text-neutral-400">{categories.length} {t('categories.totalSuffix')} {!canEdit && <span className="text-amber-600 ml-1">· {t('categories.viewOnlyFree')}</span>}</p>
        </div>
        {canEdit ? (
          ...
          <Link href="/dashboard/pricing" className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg bg-cyan-100 text-cyan-700 hover:bg-cyan-200 transition">
            <Lock className="w-3 h-3" /> {t('categories.upgradeToEdit')}
          </Link>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {[{ title: t('transactions.expense'), cats: debitCats }, { title: t('transactions.income'), cats: creditCats }].map(({ title, cats }) => (
            <div key={title} className="rounded-2xl bg-white shadow-sm overflow-hidden">
              <div className="px-3 py-2 border-b border-black/5 bg-neutral-50">
                <p className="text-xs font-semibold text-neutral-900">{title} {t('nav.categories')} <span className="text-neutral-400 font-normal">({cats.length})</span></p>
              </div>
              <table className="w-full">
                <tbody>
                  {cats.map(cat => <CatRow key={cat.id} cat={cat} />)}
                  {cats.length === 0 && <tr><td colSpan={3} className="px-3 py-4 text-center text-[10px] text-neutral-400">{t('categories.noneYet')}</td></tr>}
                </tbody>
              </table>
            </div>
          ))}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => { setShowModal(false); resetForm() }}>
          <div className="w-full max-w-xs rounded-2xl bg-white shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-black/5">
              <h2 className="text-sm font-semibold text-neutral-900">{editingCategory ? t('categories.editCategory') : t('categories.addCategory')}</h2>
              <button onClick={() => { setShowModal(false); resetForm() }}><X className="w-4 h-4 text-neutral-400" /></button>
            </div>
            <form onSubmit={handleSubmit} className="p-4 space-y-3">
              <div><label className="block text-[10px] font-medium text-neutral-400 mb-1">{t('common.name')}</label><input type="text" required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="w-full rounded-lg border border-black/10 bg-neutral-50 px-3 py-1.5 text-xs text-neutral-900 focus:outline-none focus:ring-1 focus:ring-lime-500/50" placeholder={t('categories.categoryNamePlaceholder')} autoFocus /></div>
              <div><label className="block text-[10px] font-medium text-neutral-400 mb-1">{t('common.type')}</label><select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value as any }))} className="w-full rounded-lg border border-black/10 bg-neutral-50 px-3 py-1.5 text-xs text-neutral-900 focus:outline-none"><option value="debit">{t('transactions.expense')}</option><option value="credit">{t('transactions.income')}</option><option value="both">{t('categories.both')}</option></select></div>
              <div>
                <label className="block text-[10px] font-medium text-neutral-400 mb-1">{t('time.color')}</label>
              ...
              <button type="submit" className="w-full py-2 rounded-xl bg-lime-400 hover:bg-lime-300 text-neutral-900 text-xs font-bold">{editingCategory ? t('payroll.update') : t('categories.create')}</button>
            </form>
          </div>
        </div>,
        document.body
